...
function App() {
  ...
  const clickHandle = (e) => setResult(result + e.target.name)

  const calculate = () => { try{setResult(eval(result)); } catch{ setResult("Error");}}

  const onClickOfCE = () => setResult("");
 
  const onClickOfC = () => setResult(result.slice(0, result.length - 1 ));
  return (
